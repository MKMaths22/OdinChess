...
# tests the outcomes of specific Games
class ChessTest < Minitest::Test
  def test_a_quick_white_win
    # skip
    list_of_inputs = %w[player_one player_two e2e4 e7e5 d1f3 a7a6 f1c4 h7h6 f3f7]
  ...
  def test_a_quick_black_win
    # skip
    list_of_inputs = %w[player_one player_two f2f3 e7e5 g2g4 d8h4]
    test_game = Game.new(Board.new, list_of_inputs)
    test_game.play_game
  ...
  def test_white_resignation
    # skip
    list_of_inputs = %w[player_one player_two resign]
    test_game = Game.new(Board.new, list_of_inputs)
    test_game.play_game
  ...
  def test_black_resignation
    # skip
    list_of_inputs = %w[player_one player_two e2e4 resign]
    test_game = Game.new(Board.new, list_of_inputs)
    test_game.play_game
  ...
  def test_three_fold_repitition_at_start
    # skip
    list_of_inputs = %w[player_one player_two g1f3 g8f6 f3g1 f6g8 g1f3 g8f6 f3g1 f6g8]
    test_game = Game.new(Board.new, list_of_inputs)
    test_game.play_game
  ...
  def test_three_fold_repitition_later
    # skip
    list_of_inputs = %w[player_one player_two e2e4 e7e5 g1f3 g8f6 f3g1 f6g8 g1f3 g8f6
                        f3g1 f6g8]
    test_game = Game.new(Board.new, list_of_inputs)
    test_game.play_game
    assert test_game.result.game_end_message == "That's the exact same position, for the third time. It's a 3-fold repitition draw. Well played, player_two and player_one."
  ...
  def test_insufficient_material_draw_kings_only
    # skip
    test_array = [[King.new('White'), nil, nil, nil, nil, nil, Pawn.new('White'), nil],
                  [nil, nil, nil, nil, nil, nil, King.new('Black'), nil], Array.new(8), Array.new(8), Array.new(8), Array.new(8), Array.new(8), Array.new(8)]
    test_board = Board.new(test_array, 'Black',
                           { 'White_0-0-0' => false, 'White_0-0' => false, 'Black_0-0-0' => false, 'Black_0-0' => false })
    list_of_inputs = %w[player_one player_two b7a7]
  ...
  def test_insufficient_material_draw_underpromotion
    ...
    test_board = Board.new(test_array, 'Black',
                           { 'White_0-0-0' => false, 'White_0-0' => false, 'Black_0-0-0' => false, 'Black_0-0' => false })
    list_of_inputs = %w[player_one player_two b7c7 a7a8 N]
    test_game = Game.new(test_board, list_of_inputs)
    test_game.play_game
  ...
  def test_fifty_move_rule_draw
    ...
    test_board = Board.new(test_array, 'Black',
                           { 'White_0-0-0' => false, 'White_0-0' => false, 'Black_0-0-0' => false, 'Black_0-0' => false })
    list_of_inputs = %w[player_one player_two b7c6 a7a8 Q
  ...
  def test_quickish_stalemate
    # skip
    list_of_inputs = %w[player_one player_two e2e3 a7a5 d1h5 a8a6 h5a5 h7h5 h2h4 a6h6
  ...
  def test_illegal_move_counted
    # skip
    list_of_inputs = %w[player_one player_two e2e4 e7d4]
    test_game = Game.new(Board.new, list_of_inputs)
    test_game.play_game
  ...
  def test_long_game_outcome
    # skip
    list_of_inputs = %w[Peter Chris d2e3 c2c3 f8c5 g8h6 d2d4 d7d5 d4d5 c3c5 a1a7 c1h6
                        g7h6 d1a4 c7c5 c7c6 e1c1 b1a3 c6c5 d8b6 e2e4 b6b1 b6b2 e1c1 e4e5 b2b3 e1c1 h2h4 b3c3 e1c1 e1e2 f7f5 e5d6 e5f6 b8d7 a4a7 c3d4 e2e1 e7e5 a7a8 d4h4 e1c1 f1a6 d7c5 a8b7 c8b7 g2g3 e8c8 f8d6 f2f3 e8g8 h1h4 g8h8 a6b7 c5a4 f6f7 f8e8 f7e8 Q h8g8 h8g7 h4f4 e5e4 f4f6 e4e3 e8e5 g7h8 f6f8]
    test_game = Game.new(Board.new, list_of_inputs)
    test_game.play_game
